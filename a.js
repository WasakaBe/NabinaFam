...
export default function App() {
 ...
 const getLibros = async() => {
   const respuesta = await axios.get('http://192.168.137.1/apilibro/')
   setListaLibros(respuesta.data)
}

 const addLibro = async() => {
  const obj = {nombre, edicion}
  const respuesta = await axios.post('http://192.168.137.1/apilibro/', obj)
...
const deleteLibro = async (props) => {
  const id = props.id
  const respuesta = await axios.delete('http://192.168.137.1/apilibro/?id='+id)
  alert(respuesta.data.msg)
  getLibros()
}
